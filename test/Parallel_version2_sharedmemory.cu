...
{
    int block_x = threadIdx.x % 32;
    int block_y = threadIdx.x / 32;
    int block_idx_x = blockIdx.x % 16;   //block position, 0-15
    int block_idx_y = blockIdx.x / 16;   //0-15

    __shared__ unsigned char s_data[32 * 32 * 3];
    __shared__ int s_key[32 * 32];
    for(int i = 0; i < 3; i++)
    {
        s_data[threadIdx.x + i * 1024] = bitmapImage[((block_idx_y * 32 + block_y) * 512 + (block_idx_x * 32 + block_x)) * 3 + i];
    }
    ...
    for(int i = 0; i < 3; i++)
    {
    	s_data[threadIdx.x + i * 1024] = s_data[threadIdx.x + i * 1024] ^ s_key[threadIdx.x];

    	bitmapImage[((block_idx_y * 32 + block_y) * 512 + (block_idx_x * 32 + block_x)) * 3 + i] = s_data[threadIdx.x + i * 1024];
    }
}
...
__global__ void pixel_permutation(unsigned char *bitmapImage, unsigned char *bitmapImage1, int *en_key, int *de_key)
{
    ...
    __shared__ unsigned char s_data[32 * 32 * 3];
    __shared__ unsigned char s_data_out[32 * 32 * 3];
    __shared__ int s_data_en_key[32 * 32];
    __shared__ int s_data_de_key[32 * 32];
    for(int i = 0; i < 3; i++)
    {
        s_data[threadIdx.x + i * 1024] = bitmapImage[((block_idx_y * 32 + block_y) * 512 + (block_idx_x * 32 + block_x)) * 3 + i];
    }
    ...
    s_data_de_key[(new_row - 1) * 32 + (new_col - 1)] = de_key_generator(block_x, block_y, 1024);

    for(int i = 0; i < 3; i++)
    {
    	s_data_out[(new_row - 1) * 32 + (new_col - 1) + i * 1024] = s_data[threadIdx.x + i * 1024];
    }
    __syncthreads();

    for(int i = 0; i < 3; i++)
    {
    	 bitmapImage1[((block_idx_y * 32 + block_y) * 512 + (block_idx_x * 32 + block_x)) * 3 + i] = s_data_out[threadIdx.x + i * 1024];
    }
...
int main()
{
    ...
    substitution<<<gridDim_shared, blockDim>>>(d_bitmapData, d_substitution_key);
    ...
    cudaEventRecord(start, 0);
    pixel_permutation<<<gridDim_shared, blockDim>>>(d_bitmapData, d_bitmapData1, d_pix_per_key, d_pix_per_key_de);
    cudaEventRecord(end, 0);
    cudaEventSynchronize(end);
    ...
    cudaEventRecord(start, 0);
    pixel_permutation<<<gridDim_shared, blockDim>>>(d_bitmapData4, d_bitmapData5, d_pix_per_key_de, d_pix_per_key);
    cudaEventRecord(end, 0);
    cudaEventSynchronize(end);
    ...
    cudaEventRecord(start, 0);
    substitution<<<gridDim_shared, blockDim>>>(d_bitmapData5, d_substitution_key);
    cudaEventRecord(end, 0);
    cudaEventSynchronize(end);

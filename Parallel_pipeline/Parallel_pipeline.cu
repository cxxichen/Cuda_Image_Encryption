...
__global__ void substitution(unsigned char *bitmapImage, int *sub_key,int imagewidth)
{
    ...
    for(int i = 0; i < 3; i++)
    {
		...
		bitmapImage[idx_pixel * 3 + i] = s_data[threadIdx.x * 3+i];
    }
}
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < num_streams; i++)
    {
    	...
    	for(int j = 0; j <= i; j++)
    	{
    		...
    		{
				case 0:
					substitution<<<128, blockDim, 1024, streams[0]>>>(d_input_image + offset * mem_idx,d_substitution_key, imagewidth);
					break;
				case 1:
					pixel_permutation<<<128, blockDim, 1024, streams[1]>>>(d_input_image + offset * mem_idx, d_pixel_permutation_image + offset * mem_idx,d_pix_per_key, d_pix_per_key_de, imagewidth);
					break;
    		}
    	}
    }
    ...
    for(int i = num_streams-1; i > 0; i--)
    {
    	for(int j = 0; j < i; j++)
    	{
    		...
    		{
				case 1:
					pixel_permutation<<<128, blockDim, 1024, streams[1]>>>(d_input_image + offset, d_pixel_permutation_image + offset, d_pix_per_key, d_pix_per_key_de, imagewidth);
					break;
    		}
    	}
    }
	...
	printf("Encryption substitution and pixel permutation Time: %fms\n", en_substitution_time);
